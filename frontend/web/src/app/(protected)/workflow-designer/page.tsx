...
export default function WorkflowDesignerPage() {
  ...
  function loadFromDefinition(def: WorkflowDef) {
    ...
    const newNodes: CanvasNode[] = stateList.map((state, i) => ({
      ...
      isFinal:
        state === "ARCHIVED" || state === "CANCELLED" || state === "APPROVED"
    }));
  ...
  function loadFromTemplate(template: Template) {
    try {
      const layout = template.canvasLayout
        ? JSON.parse(template.canvasLayout)
        : null;
      ...
      const newNodes: CanvasNode[] = stateNames.map((name, i) => {
        const nodeLayout = layout?.nodes?.find(
          (n: { id: string }) => n.id === name
        );
        return {
          id: name,
          type: "state" as const,
      ...
      Object.entries(transitions).forEach(([key, value]: [string, unknown]) => {
        ...
        newEdges.push({
          ...
          from: trans.from,
          to: trans.to,
          label: trans.label || trans.action || key,
          action: trans.action || key
        });
      });
      setNodes(newNodes);
      setEdges(newEdges);
  ...
  function addStateNode(stateName: string) {
    ...
    const newNode: CanvasNode = {
      ...
      color:
        STATE_COLORS[stateName] || COLOR_LIST[nodes.length % COLOR_LIST.length],
  ...
  function addNode(type: string) {
    ...
    const newNode: CanvasNode = {
      ...
      type: "state",
      label:
        type === "start" ? "DRAFT" : type === "end" ? "ARCHIVED" : "New State",
      x: 200 + Math.random() * 300,
      y: 150 + Math.random() * 200,
      color:
        type === "start"
          ...
          : type === "end"
            ? "#ef4444"
            : COLOR_LIST[nodes.length % COLOR_LIST.length],
  ...
  const handleNodeMouseDown = useCallback(
    (e: React.MouseEvent, nodeId: string) => {
      ...
      if (rect) {
        ...
        setDragOffset({
          x: e.clientX - rect.left + sl - node.x * zoom,
          y: e.clientY - rect.top + st - node.y * zoom
        });
      }
    },
  ...
  function commitEditLabel() {
    if (editingLabel) {
      setNodes((prev) =>
        prev.map((n) =>
          n.id === editingLabel ? { ...n, label: editLabelValue || n.label } : n
        )
      );
  ...
  async function saveWorkflow() {
    if (nodes.length === 0) {
      toast.error("Add at least one state");
      return;
    }
    const initialNode = nodes.find((n) => n.isInitial);
    if (!initialNode) {
      toast.error("Mark one state as Initial");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        escalationRules:
          escalationRules.length > 0 ? escalationRules : undefined
      };
  ...
  function addEscalationRule() {
    const availableStates = nodes.filter(
      (n) => !n.isFinal && !escalationRules.find((r) => r.state === n.id)
    );
    if (availableStates.length === 0) {
      toast.error("All states have rules");
      return;
    }
    setEscalationRules((prev) => [
      ...prev,
      {
        state: availableStates[0].id,
        slaHours: 24,
        maxLevel: 3,
        notifyRole: "MANAGER"
      }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Create, edit, or start from a template
            </p>
          </div>
          <button
            onClick={() => {
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-1">
                      <button
                        onClick={() => loadFromDefinition(def)}
                  ...
                  {def.escalationRules && def.escalationRules.length > 0 && (
                    <div className="mt-2 flex items-center gap-1 text-xs text-amber-600">
                      ...
                      <span>
                        {def.escalationRules.length} escalation rule
                        {def.escalationRules.length > 1 ? "s" : ""}
                      </span>
                    </div>
                  )}
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              >
                ...
                {t.estimatedDurationHours > 0 && (
                  <div className="flex items-center gap-1 text-xs text-slate-400">
                    ...
                    <span>~{t.estimatedDurationHours}h avg.</span>
                  </div>
                )}
              </button>
            ))}
            ...
            >
              ...
              <span className="font-medium text-slate-600 group-hover:text-primary-600">
                Blank Workflow
              </span>
  ...
  return (
    <div className="h-[calc(100vh-7rem)] flex flex-col">
      ...
      <div className="bg-white border border-slate-200 rounded-t-xl px-4 py-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1.5 text-sm rounded-lg flex items-center gap-1 border ${showEscalation ? "bg-amber-50 border-amber-300 text-amber-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
          >
            <AlertTriangle className="h-4 w-4" />
            Escalation
            {escalationRules.length > 0 && ` (${escalationRules.length})`}
          </button>
          <div className="h-5 w-px bg-slate-200" />
          <button
        ...
        <div className="bg-amber-50 border-x border-slate-200 px-4 py-3 space-y-2">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-amber-800 flex items-center gap-2">
              <AlertTriangle className="h-4 w-4" />
              Escalation Rules — Auto-escalate when SLA expires
            </h3>
            <button
          ...
          {escalationRules.length === 0 && (
            <p className="text-xs text-amber-600">
              No escalation rules. Workflows will not auto-escalate.
            </p>
          )}
            ...
            >
              <select
                ...
                onChange={(e) =>
                  setEscalationRules((prev) =>
                    prev.map((r, idx) =>
                      idx === i ? { ...r, state: e.target.value } : r
                    )
                  )
                }
              ...
              <input
                ...
                onChange={(e) =>
                  setEscalationRules((prev) =>
                    prev.map((r, idx) =>
                      idx === i
                        ? { ...r, slaHours: parseInt(e.target.value) || 24 }
                        : r
                    )
                  )
                }
              ...
              <input
                ...
                onChange={(e) =>
                  setEscalationRules((prev) =>
                    prev.map((r, idx) =>
                      idx === i
                        ? { ...r, maxLevel: parseInt(e.target.value) || 3 }
                        : r
                    )
                  )
                }
              ...
              <input
                ...
                onChange={(e) =>
                  setEscalationRules((prev) =>
                    prev.map((r, idx) =>
                      idx === i ? { ...r, notifyRole: e.target.value } : r
                    )
                  )
                }
                className="w-28 text-xs border border-slate-200 rounded px-2 py-1"
                placeholder="MANAGER"
              />
              <button
                onClick={() =>
      ...
      <div className="flex flex-1 overflow-hidden border-x border-b border-slate-200 rounded-b-xl">
        ...
        <div className="w-56 bg-slate-50 border-r border-slate-200 p-3 space-y-3 overflow-y-auto">
          ...
          <div className="pt-3 border-t border-slate-200">
            ...
            <button
              onClick={() => {
                if (connectingFrom) {
                  ...
                  toast("Click a source node, then a target node");
                  setConnectingFrom("__awaiting__");
                }
              }}
            ...
            >
              ...
              {connectingFrom ? "Cancel Connect" : "Add Connection"}
            </button>
          </div>
              ...
              return (
                <div className="pt-3 border-t border-slate-200 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={node.isInitial || false}
                      onChange={(e) =>
                        setNodes((prev) =>
                          prev.map((n) =>
                            n.id === selectedNode
                              ? { ...n, isInitial: e.target.checked }
                              : n
                          )
                        )
                      }
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={node.isFinal || false}
                      onChange={(e) =>
                        setNodes((prev) =>
                          prev.map((n) =>
                            n.id === selectedNode
                              ? { ...n, isFinal: e.target.checked }
                              : n
                          )
                        )
                      }
                  ...
                  <button
                    ...
                    className="w-full text-xs py-1 px-2 bg-blue-50 text-blue-600 rounded border border-blue-200 hover:bg-blue-100"
                  >
                    Connect From Here
                  </button>
                  <button
                    onClick={() => deleteNode(selectedNode)}
                    className="w-full text-xs py-1 px-2 bg-red-50 text-red-600 rounded border border-red-200 hover:bg-red-100"
                  >
                    Delete State
                  </button>
                </div>
        ...
        <div
          ref={canvasRef}
          className="flex-1 relative overflow-auto bg-[#f8fafc]"
        ...
        >
          <div
            style={{
              width: canvasExtent.w * zoom,
              height: canvasExtent.h * zoom,
              minWidth: "100%",
              minHeight: "100%",
              position: "relative",
              backgroundImage:
                ...
                return (
                  <g key={edge.id}>
                    <line
                      ...
                      x2={to.x - offsetX}
                      y2={to.y - offsetY}
                      stroke={selectedEdge === edge.id ? "#3b82f6" : "#94a3b8"}
            ...
            >
              {nodes.map((node) => (
                <div
                  ...
                  onDoubleClick={() => startEditLabel(node.id)}
                >
                  <div
                    className="rounded-xl px-4 py-3 shadow-sm border-2 bg-white"
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center text-slate-400">
                  ...
                  <p className="text-sm mt-1">
                    Add states from the palette on the left
                  </p>
                </div>
              </div>
            )}
