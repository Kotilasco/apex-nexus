...
public class EwsClient {
    ...
    private List<EwsMessage> parseMessages(String xml) throws Exception {
        ...
        for (int i = 0; i < items.getLength(); i++) {
            ...
            if (subjs.getLength() > 0) subject = subjs.item(0).getTextContent();
            NodeList atts = el.getElementsByTagNameNS(TYPES_NS, "HasAttachments");
            if (atts.getLength() > 0) hasAtt = "true".equalsIgnoreCase(atts.item(0).getTextContent());
            NodeList emails = el.getElementsByTagNameNS(TYPES_NS, "EmailAddress");
            if (emails.getLength() > 0) from = emails.item(0).getTextContent();

            if (!itemId.isBlank()) messages.add(new EwsMessage(itemId, changeKey, subject, from, hasAtt));
        }
        return messages;
    }
    ...
    private List<EwsAttachmentInfo> parseAttachmentInfoList(String xml) throws Exception {
        ...
        for (int i = 0; i < fileAtts.getLength(); i++) {
            ...
            if (ids.getLength() > 0) id = ((Element) ids.item(0)).getAttribute("Id");
            ...
            if (cts.getLength() > 0) ct = cts.item(0).getTextContent();
            NodeList sizes = el.getElementsByTagNameNS(TYPES_NS, "Size");
            if (sizes.getLength() > 0) try { size = Long.parseLong(sizes.item(0).getTextContent()); } catch (NumberFormatException ignored) {}
            if (!id.isBlank()) list.add(new EwsAttachmentInfo(id, name, ct, size));
        }
        return list;
    }
    ...
    private EwsAttachmentContent parseAttachmentContent(String xml) throws Exception {
        Document doc = parseXml(xml);
        NodeList fileAtts = doc.getElementsByTagNameNS(TYPES_NS, "FileAttachment");
        if (fileAtts.getLength() == 0) throw new RuntimeException("No FileAttachment in EWS response");
        ...
        if (contents.getLength() > 0) data = Base64.getDecoder().decode(contents.item(0).getTextContent().replaceAll("\\s+", ""));
